...
implementations, debuggers, test and verification infrastructure,
...
reads two registers, the second one is specified in the rs2 field; the
...
\begin{figure}[htbp]
  ...
  \caption{\label{Fig_J_imm}
           Construction of 21-bit immediate from 20 ``imm'' bits in J-type instructions}
\end{figure}

Similarly, Figure~\ref{Fig_B_imm} shows how the 12 ``imm'' bits in a
B-type instruction are permuted, with a 0 bit appended to the right,
to produce a 13-bit immediate value to be used in the computation.
\begin{figure}[htbp]
  \centerline{\includegraphics[width=6in,angle=0]{Figures/Fig_B_imm}}
  \caption{\label{Fig_B_imm}
           Construction of 13-bit immediate from 12 ``imm'' bits in B-type instructions}
\end{figure}
...
% \centerline{\frame{\includegraphics[width=5in,angle=0]{Figures/Fig_LUI_AUIPC}}}
  \centerline{\includegraphics[angle=0]{RSN_2026-06.004.00_RISCV_ISA_LUI_AUIPC}}
  \caption{\label{Fig_LUI_AUIPC} RISC-V LUI and AUIPC Instruction semantics}
\end{figure}
(Please ignore the first first two lines in the figure about the SRLI
and SRAI instructions which are discussed in Section~\ref{sec:SRxI}.)
...
in which their cache lines are written back to memory has no
relationship to the program STORE order).
...
the Privileged ISA Specification document).
...
\begin{figure}[htbp]
% OLD \centerline{\includegraphics[width=6in,angle=0]{Figures/Fig_Instr_Exec}}
