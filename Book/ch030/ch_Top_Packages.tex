...
simulation, and ``\verb|-o|'' tells it the desired name for the final
executable.
...
{\footnotesize
\begin{Verbatim}[frame=single, numbers=left, label=in file Ex-03-B-Top-and-DUT/DUT.bsv]
package DUT;
...
{\footnotesize
\begin{Verbatim}[frame=single, numbers=left, label=in file Ex-03-B-Top-and-DUT/Top.bsv]
package Top;
...
and then run the executable:\footnote{
Note: Bluesim seems to print ``{\tt <space>2}'' for ``{\tt 02}''.
}
...
The latter case is useful for defining a so-called ``\emph{abtract
data type}'', {\ie} a type that is known outside the package but whose
...
{\footnotesize
\begin{Verbatim}[frame=single, numbers=left,
                 label=in file Ex-03-C-Module-and-Interface/DUT.bsv]
package DUT;
...
{\footnotesize
\begin{Verbatim}[frame=single, numbers=left,
                 label=in file Ex-03-C-Module-and-Interface/Top.bsv]
package Top;
...
\begin{itemize}
 ...
 \item Each rule ``firing'' is \emph{atomic}---all its state updates
       ...
       {\ie} there is never any interleaving of actions in one rule
       with actions in another rule.
