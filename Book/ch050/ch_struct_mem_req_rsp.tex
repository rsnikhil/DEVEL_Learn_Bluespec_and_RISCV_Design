...
distinguish this for debugging.}  We could define an enum type for
this:
...
which will use a 3-bit encoding (0 for FETCH, 1 for LOAD, 2 for STORE,
3 for FENCE and 4 for FENCE\_I).  However, we look to the future where
we might extend {\FIFE} and {\DRUM} to implement the ``A'' extension
(for AMO or ``Atomic Memory Operations'').  The RISC-V ISA
Unprivileged Spec document, Chapter 8, describes additional memory
operations LR (Load-Reserved), SC (Store-Conditional), AMOSWAP,
AMOADD, AMOXOR, AMOAND, AMOOR, AMOMIN, AMOMAX, AMOMINU, and AMOMAXU,
each of which comes in a 32-bit version (in RV32 and RV64) and a
64-bit version (in RV64).  These operations are coded with 5 bits in
the instruction (\verb|instr[31:27]|, which we call ``{\tt funct5}'').
