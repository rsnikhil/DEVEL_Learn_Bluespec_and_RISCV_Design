...
32'h3             // Hex literal, 32 bits wide
...
The syntax of an identifier (name) in {\BSV} follows the same
conventions as in many programming languages: any sequence of
alphabets, digits and underscore characters, with the first letter
...
\verb|OPCLASS_CONTROL|, \verb|OPCLASS_INT|, \verb|OPCLASS_MEM| and
\verb|OPCLASS_FENCE| are also value constants.
...
{\bsc} can resolve it statically.  This is a somewhat advanced
...
The simplest and lowest-level type in {\BSV} is the bit-vector (a
vector made up of a particular number of bits).  It is
``lowest-level'' in that it is the only type that directly maps into
hardware and can be observed directly in hardware with a waveform
viewer or an oscilloscope.  Later we will see that in {\BSV} one can
define more abstract types such as integers, booleans, vectors and
arrays, lists, structs (records), tagged unions (algebraic types),
trees, and so on.  These are all language-level abstractions, {\ie} a
language-level view of bits; ultimately, all values are represented in
hardware as bit-vectors.
...
As explained in Section~\ref{BSV_hex_bin_literals}, the literal
constant \verb|'h_1000| is of unspecified size (as opposed to
\verb|32'h_1000| which has precisely 32 bits).  Technically, the
literal constant has {\BSV} type {\tt Integer}, for true, unbounded,
mathematical integers (see also Section~\ref{BSV_ints}).  {\bsc}
implicitly applies the {\tt fromInteger} function to this constant,
which converts it from {\tt Integer} to the type required by the
context, here \verb|Bit#(32)|.  If the constant is too large for the
context, {\bsc} will not silently truncate it; it will give an
...
``0'', for example, the {\bsc} compiler has to create circuitry
ensuring that field's value is 0. By saying ``\verb|?|'', the
{\bsc} compiler is allowed to pick any value that minimizes
circuitry.

Third, in places where it does not result in additional hardware, the
{\bsc} compiler usually injects the specific value
...
{\ie} {\bsc}'s type-checking will complain if one is used where
...
\index[BSV]{UInt@{\tt Integer}!Unbounded Integer type (non-synthesizable)}
...
\verb|Integer| is used only for compile-time integers, such as a
verbosity level for debugging, or the size of a vector of interfaces.
...
\EXERCISE{Ex-04-C-Bit-Vectors-Operations}

\EXERCISE{Ex-04-D-Decode-Imm}
...
In {\BSV}, and Verilog/SystemVerilog RTL, we consider such operators
as ``primitive''.  In fact, such operators must themselves be
implemented using lower-level circuit primitives such as AND, OR, and
NOT gates which, in turn, must be implemented with even lower-level
circuit structures such as transistors.  We do not concern ourselves
with such lower-level implementation because nowadays this is
performed for us automatically by excellent so-called ``synthesis''
tools.
...
\index[BSV]{ActionValue@{\tt ActionValue}!Type of experssion with side-effect and return value}
...
\index[BSV]{Types!``value types'' (not {\tt Action} or {\tt ActionValue})}
...
\EXERCISE{Ex-04-F-FSM-Testbench}
...
Because we said ``\verb|deriving(Bits)|'', the {\bsc} compiler
...
has a mechanism by which we can specify this, but we don't discuss
this further here because it involves a more advanced {\BSV}
capability called ``typeclasses'' and ``typeclass instances''.
...
Because we said ``\verb|deriving(Eq)|'', the {\bsc} compiler will
...
declararion, the {\bsc} compiler will automatically define an
...
which, in turn, is used to select one of two 3-bit constant values,
producing a final 3-bit result.  The multiplexer, also called a
...
Just like in Verilog, SystemVerilog and C/C++, the {\bsc} compiler
...
When we invoke the {\bsc} compiler, we can pass it command line
