...
In the source code you will see that this struct defines some
additional fields: {\tt predicted\_pc}, {\tt epoch} and {\tt
halt\_sentinel}.  Please ignore these for now; the first two are not
used in {\DRUM}, only in {\FIFE}, and the latter is used only if we
have hardware debugger support.

\vspace{1ex}

In the source code you will also see an additional field: {\tt xtra},
of type {\tt Fetch\_to\_Decode\_Xtra}.  This, too, can be ignored for
now.  For many a struct of type $X$ we carry some additional
information in a field of type $X${\tt{}\_Xtra}, that is \emph{only}
used for debugging and otherwise are not necessary in a production
implementation.  To avoid visual clutter in the production code, we
choose to define these in a separate file that is inserted into this
file (see {\tt `include "Inter\_Stage\_Xtra.bsvi"}) near the top of
this file).  This is just a stylistic choice; all the fields could
have been defined directly within the {\tt Fetch\_to\_Decode} struct.
Also, in a production design, we can easily remove these additional
fields to eliminate the wires and registers that they occupy.

\vspace{1ex}

In this case, the type {\tt Fetch\_to\_Decode\_Xtra} is a struct with
one field, {\tt inum}, that holds the ``instruction number'', which is
a sequence number counting every instruction fetched.  In {\DRUM} this
number is exactly the number of insructions executed.  In {\FIFE} this
number will be larger because some instructions are fetched
speculatively and discarded (note executed).
...
Line 18 extracts a 5-bit (for RV32) or 6-bit (for RV64) ``shift
