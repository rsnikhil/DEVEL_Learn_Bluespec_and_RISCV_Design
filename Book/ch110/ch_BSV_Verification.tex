...
A {\tt dynamicAssert} is no more than an if-$B$-then-$A_1$-else-$A_2$ where
\begin{tightlist}
  \item $B$ is the assertion condition
  \item Action $A_1$ is {\tt noAction} (do nothing if the condition is true)
  \item Action $A_2$ is {\tt{}\$display(}\emph{string-to-be-printed}{\tt{})}
        followed by a {\tt \$finish} to terminate the simulation.
\end{tightlist}

In fact, the if-then-else version is often more convenient because we
can write {\tt \$display} to also print current values of interest,
instead of just a {\tt String}.
